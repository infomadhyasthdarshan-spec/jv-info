...
const DownloadResources = () => {
  ...
  ].filter(Boolean); // remove undefined rows if any
  ...
  return (
    <div className="bg-gray-100 py-8 px-4 md:px-0 max-w-5xl mx-auto">
      <h2 className="page-heading font-bold text-black mb-6 ssr-hading">
        {text.Resourcepage.downloadResources.title}
      </h2>
                ...
                {section.items.map((item, idx) => {
                  if (item.isTitle) {
                    return (
                      <li
                        key={idx}
                        className="px-5 py-3 bg-[#FFF5E7] page-common-md-pera"
                      >
                        {item.text}
                      </li>
                    );
                  } else {
                    // Parse anchor from text
                    const tempDiv = document.createElement("div");
                    tempDiv.innerHTML = item.text;
                    const anchor = tempDiv.querySelector("a");
                    const href = anchor?.getAttribute("href");
                    const linkText = anchor?.innerHTML;
                    ...
                    return (
                      <li key={idx}>
                        ...
                        >
                          <span
                            className="page-collapse-link flex-1"
                            dangerouslySetInnerHTML={{ __html: wrapHindiInHtmlString(linkText) }}
                          ></span>
                          {item.isIcon && (
